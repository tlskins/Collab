class CommentsController < ApplicationController
  ...
  def create
    ...
    if params[:comment][:parent_id].to_i > 0
      ...
      @comment = parent.children.build(parent_id: params[:comment][:parent_id], body: params[:comment][:body], branch_id: params[:branch_id])
    else
      @comment = branch.comments.new(comment_params)
    end
